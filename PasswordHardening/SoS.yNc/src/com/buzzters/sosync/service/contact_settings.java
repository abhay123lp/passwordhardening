	...
	public void onStart(Intent intent, int starId)
	{
		while(true)
		{
			...
			while(!c.isAfterLast())
			{
				...
				System.out.println("calendar event ->"+calendar_event);
				...
				Date sys=null;
				try {
					d1 = (Date)sdf.parse(start_time);
					d2=(Date)sdf.parse(end_time);
					sys=new Date();
				...
				System.out.println("starttime->"+d1);
			
				if((sys.after(d1))&&(sys.before(d2)))
				{
				...
				System.out.println("Rule selected ->"+rule_action);
